...
- **Control Físico de Tapa (Servo SG90)**: Integración de la librería `gpiozero` (`lgpio`) en `backend/servo_controller.py` mediante el GPIO 19 de la Raspberry Pi. Los ángulos de apertura y cierre del servo se configuran de forma centralizada desde `printer_config.yaml` (sección `servo`: `angulo_abierto` y `angulo_cerrado`), consumidos a través de `PrinterConfig.SERVO_ANGULO_ABIERTO` / `SERVO_ANGULO_CERRADO`. No existen valores hardcodeados en el código.
...
- **3 Nuevas Figuras 3D Decorativas**: Tres figuras 3D multicapa disponibles junto al Cilindro original:
  - **Cilindro Domo**: Cilindro con cúpula/domo convexo. Las capas base son circulares de radio completo; las capas superiores siguen un perfil `cos(t^convexidad × π/2)` con Z acelerado (`z_aceleracion_domo`), produciendo un domo pronunciado que mantiene su ancho antes de cerrarse. Ambos parámetros son configurables desde `printer_config.yaml`.
